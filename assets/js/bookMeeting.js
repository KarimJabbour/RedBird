...
async function fetchAvailableDates() {
    try {
        const response = await fetch("bookMeeting.php?ID=58");
...
function displayTimeForDate(selectedDate) {
    ...
    const selectedDayElement = document.querySelector(`.day[data-date='${selectedDate}']`);
...
function populateBookingDetails(data) {
    ...
    titleElement.querySelector("h2").textContent = data.BookingName || "Untitled Booking";

    const detailsElement = document.getElementById("booking-details");
    detailsElement.querySelector("p").textContent = data.Details || "No additional details provided.";

    const locationElement = document.getElementById("booking-location");
    locationElement.querySelector("p").textContent = data.Location || "No location specified.";
}

...
function getDates(bookingData) {
    const meetingDates = bookingData.MeetingDates ? bookingData.MeetingDates.trim().replace(/^"|"$/g, '').split(',') : [];
    const startTimes = bookingData.StartTimes ? bookingData.StartTimes.trim().replace(/^"|"$/g, '').split(',') : [];
    const endTimes = bookingData.EndTimes ? bookingData.EndTimes.trim().replace(/^"|"$/g, '').split(',') : [];
    console.log("meeting dates"+meetingDates);

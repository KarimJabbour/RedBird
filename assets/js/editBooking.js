...
function loadBookingData(bookingData) {
    ...
    document.getElementById("location").value = bookingData.Location === -1 ? "" : (bookingData.Location || "");
    document.getElementById("details").value = bookingData.Details || "";
    document.getElementById("max-attendees").value = bookingData.MaxAttendees === -1 ? "" : (bookingData.MaxAttendees || "");
    document.getElementById("time-slot").value = bookingData.TimeSlotLength === -1 ? "": (bookingData.TimeSlotLength || "");
    document.getElementById("attachment-link").value = bookingData.Attachments === "-1" ? "" : (bookingData.Attachments || "");
    document.getElementById("meeting-link").value = bookingData.MeetingLink === "-1" ? "" : (bookingData.MeetingLink || "");

    document.getElementById("start-date").value = bookingData.StartRecurringDate || "";
    document.getElementById("end-date").value = bookingData.EndRecurringDate || "";

    const recurringTimelineSelect = document.getElementById("recurring-timeline");
    const optionToSelect = recurringTimelineSelect.querySelector(`option[value="${bookingData.RecurrenceFrequency}"]`);
    if (optionToSelect) optionToSelect.selected = true;

    const recurrenceFrequency = bookingData.RecurrenceFrequency;
    const recurrenceDays = bookingData.RecurrenceDays ? bookingData.RecurrenceDays.split(',') : [];
    if (recurrenceFrequency === 'weekly' || recurrenceFrequency === '2weekly' || recurrenceFrequency === '4weekly') {
...
function getDates(bookingData) {
    meetingDates = bookingData.MeetingDates ? bookingData.MeetingDates.trim().replace(/^"|"$/g, '').split(',') : [];
    startTimes = bookingData.StartTimes ? bookingData.StartTimes.trim().replace(/^"|"$/g, '').split(',') : [];
    endTimes = bookingData.EndTimes ? bookingData.EndTimes.trim().replace(/^"|"$/g, '').split(',') : [];
    console.log("meeting dates"+meetingDates);
    // Update global highlighted array
    highlighted = meetingDates.map(date => date.trim());
    meetingDates.forEach(meeting => {
        manualAdjustments.add(meeting);
        //generateManualTimeCard(meeting);
    })
    updateHighlightedDates();
    //highlightAvailableDates(highlighted);
    generateTimeCards(meetingDates, startTimes, endTimes, currentMonth,currentYear);
    updateDayTimesFromCards();
...
function generateTimeCards(meetingDates, startTimes, endTimes, month, year) {
    ...
    meetingDates.forEach((date, index) => {
        ...
        startTimeLabel.innerHTML = `
            <b>Start Time:</b>
            <input type="time" class="time-slot start-time" data-date="${date}" value="${startTimes[index] || ''}" required>
        `;
        timeSlotRow.appendChild(startTimeLabel);
        ...
        endTimeLabel.innerHTML = `
            <b>End Time:</b>
            <input type="time" class="time-slot end-time" data-date="${date}" value="${endTimes[index] || ''}" required>
        `;
        timeSlotRow.appendChild(endTimeLabel);
...
//         // Gets rid of any earlier selected dates when start is updated
//         const start = document.getElementById("start-date").value;
//         manualAdjustments.forEach(date => {
//             if ((new Date (date) < new Date(start))) {
//                 manualAdjustments.delete(date);
...
//         // Gets rid of any later selected dates when end is updated
//         const end = document.getElementById("end-date").value;
//         manualAdjustments.forEach(date => {
//             if ((new Date (date) > new Date(end))) {
//                 manualAdjustments.delete(date);
...
//         timeCardContainer.insertBefore(timeCard, note);
//         //timeCardContainer.appendChild(timeCard);
//         applyDefaultTimesToTimeCard(timeCard);
//     }
// }
...
//     //generateTimeCards(highlighted, startTimes, endTimes, currentMonth, currentYear);
// }
